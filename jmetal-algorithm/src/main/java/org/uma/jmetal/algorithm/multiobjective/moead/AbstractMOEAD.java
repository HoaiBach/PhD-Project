...
public abstract class AbstractMOEAD<S extends Solution<?>> implements Algorithm<List<S>> {
	...
	public enum FunctionType {TCHE, PBI, AGG, InvertPBI}
	...
	protected int stepIteration;

	public AbstractMOEAD(Problem<S> problem, int populationSize, int resultPopulationSize,
			int maxIterations, CrossoverOperator<S> crossoverOperator, MutationOperator<S> mutation,
		...
		stepIteration = this.maxIterations/10;
	...
	protected void updateNeighborhoodTopology(){
		//first calculate the array of distances between subpopulation-solutions
		FeatureSelection fs = (FeatureSelection)problem;
		double[][] distances = new double[this.populationSize][this.populationSize];
		int[][] idxs = new int[this.populationSize][this.populationSize];
		for(int i=0;i<this.populationSize;i++){
			idxs[i][i]= i;
			DoubleSolution si = (DoubleSolution) this.population.get(i);
			for(int j=i+1;j<this.populationSize;j++){
				//initialise idxs
				idxs[i][j] = j;
				idxs[j][i] = i;

				DoubleSolution sj =  (DoubleSolution) this.population.get(j);
				double distance=0;
				for(int fIndex =0;fIndex<si.getNumberOfVariables();fIndex++){
					double ivalue = si.getVariableValue(fIndex);
					double jvalue = sj.getVariableValue(fIndex);
					if((ivalue > fs.getThreshold() && jvalue <= fs.getThreshold())
							|| (ivalue <= fs.getThreshold() && jvalue >fs.getThreshold())){
						distance+=1;
					}
				}
		...
		for(int i=0;i<populationSize;i++){
			MOEADUtils.minFastSort(distances[i], idxs[i], populationSize, neighborSize);
			System.arraycopy(idxs[i], 0, neighborhood[i], 0, neighborSize);
		}
	}
	...
	public void setReferencePoint(double[] refPoint){
		if(refPoint.length != this.problem.getNumberOfObjectives()){
			System.out.println("The reference point must have same length as the number of objectives!!!");
			System.exit(0);
		}
		for(int i=0;i<this.problem.getNumberOfObjectives();i++){
			idealPoint[i] = refPoint[i];
		}
	}
	...
	public void setNadirPoint(double[] naPoint){
		if(naPoint.length != this.problem.getNumberOfObjectives()){
			System.out.println("The reference point must have same length as the number of objectives!!!");
			System.exit(0);
		}
		for(int i=0;i<this.problem.getNumberOfObjectives();i++){
			this.nadirPoint[i] = naPoint[i];
		}
	}
	...
	protected void updateExternalPopulation(){
		for(S ind: population){
			externalPopulation.add((S) ind.copy());
		}
		externalPopulation = SolutionListUtils.getNondominatedSolutions(externalPopulation);
	}
	...
	double fitnessFunction(S individual, double[] lambda) throws JMetalException {
		...
		if (MOEAD.FunctionType.TCHE.equals(functionType)) {
			...
			fitness = (d1 + theta * d2);
		} else if(MOEAD.FunctionType.InvertPBI.equals(functionType)){
			double d1, d2, nl;
			double theta = 5.0;
...
//		return SolutionListUtils.getNondominatedSolutions(population) ;
		return this.externalPopulation;
	}

	public void setSeed(long seeder){
		this.randomGenerator.setSeed(seeder);
	}
